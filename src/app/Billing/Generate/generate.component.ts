...
export class GenerateBillComponent implements OnInit {
  ...
  getConsultationLabel(item: LookupConsultation): string {
    ...
    const ageSex = [item.patient?.age ?? "", item.patient?.gender || ""]
      ...
      .join("/");
    const when =
      `${this.formatDisplayDate(item.appointment?.date)} ${item.appointment?.displayTime || ""}`.trim();
  ...
  private applyConsultation(
    item: LookupConsultation,
    visitDate?: string,
  ): void {
    ...
    if (item.appointment?.consultationType) {
      this.consultationType =
        item.appointment.consultationType === "Video" ? "Online" : "In-Person";
    }
  ...
  dialogPrintBill(): void {
    ...
    const invoice = document.querySelector(
      ".generate-print-invoice .invoice",
    ) as HTMLElement | null;

    if (!invoice) {
      alert("Unable to prepare bill for printing. Please try again.");
      return;
    }
  ...
  dialogDownloadBill(): void {
    ...
    if (!invoice) {
      alert("Unable to prepare bill for download. Please try again.");
      return;
    }

    this.billPdfService
      .downloadInvoiceElement(
        invoice,
        `${this.savedBillData.billNo || "bill"}.pdf`,
      )
      .catch(() => {
